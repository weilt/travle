...
import com.common.vo.LoginRedisVo;
		...
		switch (validateType) {
			case NULL: //判断为空
				if(ObjectHelper.isEmpty(o)) {
					if(msg.length < i+1) {
						throw new BaseAdminException("参数不能为空! param is " + parameter[i]);
					} else {
						throw new BaseAdminException(msg[i]);
					}
				}
				break;
			case EMAIL: //验证邮箱
				if(ObjectHelper.isNotEmpty(o) && !ObjectHelper.checkEmail(o)) {
					if(msg.length < i+1) {
						throw new BaseAdminException("邮箱格式错误! param is " + parameter[i]);
					} else {
						throw new BaseAdminException(msg[i]);
					}
				}
			...
			case PHONE:
				if(ObjectHelper.isNotEmpty(o) && !ObjectHelper.isPhoneLegal(o)) {
					if(msg.length < i+1) {
						throw new BaseAdminException("号码格式错误! param is " + parameter[i]);
					} else {
						throw new BaseAdminException(msg[i]);
					}
				}
			...
			case IdCard:
				if(ObjectHelper.isNotEmpty(o) && !ObjectHelper.checkIdCard(o)) {
					if(msg.length < i+1) {
						throw new BaseAdminException("身份证格式错误! param is " + parameter[i]);
					} else {
						throw new BaseAdminException(msg[i]);
					}
				}
			...
			case Digit:
				if(ObjectHelper.isNotEmpty(o) && !ObjectHelper.checkDigit(o)) {
					if(msg.length < i+1) {
						throw new BaseAdminException("数字类型错误! param is " + parameter[i]);
					} else {
						throw new BaseAdminException(msg[i]);
					}
				}
			...
			case Decimals:
				if(ObjectHelper.isNotEmpty(o) && !ObjectHelper.checkDecimals(o)) {
					if(msg.length < i+1) {
						throw new BaseAdminException("小数类型错误! param is " + parameter[i]);
					} else {
						throw new BaseAdminException(msg[i]);
					}
				}
			...
			case TOKEN:
				if(ObjectHelper.isEmpty(o)) {
					if(msg.length < i+1) {
						throw new BaseAdminException("请求参数异常! param is " + parameter[i]);
					} else {
						throw new BaseAdminException(msg[i]);
					}
				}
				if(ObjectHelper.isEmpty(LoginRedisVo.find(o))) {
