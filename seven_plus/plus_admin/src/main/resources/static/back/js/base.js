...
function validation(parameter,type,msg){
	for (var i = 0; i < parameter.length; i++) {
		...
		if(validateType == null||validateType == ''||validateType == 'NULL' ){
			if(o.length == 0 || !o.trim() ){
				if(msg.length < i+1) {
		...
		}else if('Image' == validateType){  //验证图片类型
			if(o.length > 0 && !PHONE(o)){
				var index = o.lastIndexOf(".");  
				var strs = o.substring(index + 1, o.length);
				if(strs!='JPG' && strs!='JPEG' && strs!='PNG' && strs!='GIF' && strs!='jpg' && strs!='jpeg' && strs!='png' && strs!='gif'){
					if(msg.length < i+1) {
						layer.msg("该文件不是图片! param is " + parameter[i]);
					} else {
						layer.msg(msg[i]);
					}
					$('#'+parameter[i]).focus();
		...
		}else if('Space' == validateType){  //验证字符串包含空格类型
			if(o.length > 0 && !PHONE(o)){
				if(o.indexOf(" ")!=-1){
					if(msg.length < i+1) {
						layer.msg("密码不可以包含空格! param is " + parameter[i]);
					} else {
						layer.msg(msg[i]);
					}
					$('#'+parameter[i]).focus();
		...
		}else if("balance" == validateType){
			if(o<0){
				if(msg.length < i+1) {
					layer.msg("金额不可以小于0! param is " + parameter[i]);
				} else {
					layer.msg(msg[i]);
				}
				$('#'+parameter[i]).focus();
