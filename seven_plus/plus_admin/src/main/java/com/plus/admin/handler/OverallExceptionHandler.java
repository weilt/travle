package com.plus.admin.handler;
...
public class OverallExceptionHandler {
    ...
    public Object exception(Exception e) {
        ...
        if (e instanceof BaseException) {
            //自定义异常，用于处理自己的异常数据信息
            return ResultEntity.build(((BaseException) e).getCode() == null ? ResultCode.SYS_ERROR.code : ((BaseException) e).getCode(), msg);
        } else if (e instanceof BaseAdminException) {
            return new JsonResult(ResultCode.ERROR.code,msg);
        } else {
            ResultCode resultCode = ResultCode.getResultCode(msg);
            if (resultCode == null) {
                logger.error("系统异常：", e.getMessage());
                return ResultEntity.build(ResultCode.SYS_ERROR.code, msg);//系统错误

            } else {
                return ResultEntity.build(resultCode);
            }
        }
    }
    ...
    private static String formatException(Exception e){
        ...
        //一些常见异常提示
        if (e instanceof BaseException) {  //自定义异常，用于处理当前自己的异常数据信息
