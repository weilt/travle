package com.hx.user.logs.service;
...
import com.hx.user.logs.annotation.InsertIncId;
import com.hx.user.logs.entity.parent.Logs;
import com.hx.user.logs.exception.LogsException;
 ...
 */
@Service
public class LogseeService {

    private final static Logger log = LoggerFactory.getLogger(LogseeService.class);
    ...
    private void insertIncId(Logs t, String tableName) throws LogsException {
        ...
        if(StringUtils.isBlank(incidName)) throw new LogsException( "class "+className+" is not find annotation InsertIncId;");
            ...
            try {
                MongoService.insertIncId(MONGO_DB_NAME,className,t,"logId");
            } catch (Exception e) {
