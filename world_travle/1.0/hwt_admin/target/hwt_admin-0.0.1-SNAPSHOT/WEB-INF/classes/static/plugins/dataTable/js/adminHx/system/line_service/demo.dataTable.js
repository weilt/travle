	...
	_settings.ro_columns =  [
        ...
        {title:'操作', customHtml:
        	...
        	function(rowData,index){
				var html = '';
				if (rowData.is_hide==0){
        			html += '<a onclick="deleteAndRecoveryById(1,'+rowData.attribute_id+',\'adminHx/line_service/delete\',\'确定要删除服务---'+rowData.attribute_name+'？\')" href="javascrip:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-trash"></i>删除</div></a>';
        		}else{
        			html += '<a onclick="deleteAndRecoveryById(0,'+rowData.attribute_id+',\'adminHx/line_service/delete\',\'确定要恢复服务---'+rowData.attribute_name+'？\')" href="javascrip:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>恢复</div></a>';
        		}
				html += '<a onclick="edit_attribute('+rowData.attribute_id+',\'adminHx/line_service/update\',\'线路服务-编辑（'+rowData.attribute_name+'）\',1)" href="javascrip:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>修改</div></a>';
				return html;
        	}
        }	
    ];
