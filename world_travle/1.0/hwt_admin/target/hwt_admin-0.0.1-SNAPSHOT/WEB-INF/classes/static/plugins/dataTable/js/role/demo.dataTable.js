	...
	_settings.ro_columns =  [
        ...
        {title:'操作', customHtml:
        	...
        	function(rowData,index){
				...
				if (rowData.id!=1){
					html += '<a onclick="deleteById(this,'+rowData.id+',\'admin/role/delete\',\'确定要删除角色---'+rowData.name+'？\')" href="javascrip:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-trash"></i>删除</div></a>';
					html+='<a onclick="adpower(this,1,'+rowData.id+',\'admin/role/adpower\',\'角色授权---'+rowData.name+'\')" href="javascrip:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>授权</div></a>'
				}
				return html;
        	}
        }	
