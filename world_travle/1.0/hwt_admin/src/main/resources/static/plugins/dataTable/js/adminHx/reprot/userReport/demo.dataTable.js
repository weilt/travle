...
$(function(){
	...
	// 请求地址
	_settings.ro_requestUrl = "adminHx/userReport/query";
	// 排序设置
	...
	_settings.ro_columns =  [
        ...
        /*{title:'操作', customHtml:
        	...
        	function(rowData,index){
				...
				html += '<a onclick="userFriend('+rowData.user_id+',\'adminHx/user/userFriend\',\'用户好友-查看（'+rowData.account+'）\')" href="javascrip:void(0);" class="btn btn-sm btn-outline-primary mg-r-5 pd-b-5"><div><i class="fa fa-pencil"></i>用户好友</div></a>';
				return html;
        	}
        }	*/
    ];
...
function paramBuffer() {
	_settings.ro_queryParams =  function createParams(){
		...
		if(isNotEmpty($("#paramMap_registerDate").val()))
			params.paramMap.registerDate = $("#paramMap_registerDate").val();
