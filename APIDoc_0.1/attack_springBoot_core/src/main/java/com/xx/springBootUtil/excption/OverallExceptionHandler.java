...
public class OverallExceptionHandler{
    ...
    public Object exception(HttpServletRequest request,HttpServletResponse response, Exception e) {
    	...
    	if (e instanceof BaseException){ 
    		//自定义异常，用于处理自己的异常数据信息
    		jsonResult = new JsonResult(ResultCode.ERROR.code,msg,ResultCode.ERROR.msg);
    	} else if (e instanceof ValidateException){
    		//请求参数错误异常
    		jsonResult = new JsonResult(ResultCode.REQUEST_PRAM.code,msg,ResultCode.REQUEST_PRAM.msg);
    	} else {
    		ResultCode resultCode = ResultCode.getResultCode(msg);
    		 if(resultCode == null) {
    			 e.printStackTrace();
    			 jsonResult = new JsonResult(ResultCode.SYS_ERROR.code,msg,ResultCode.SYS_ERROR.msg);//系统错误
    		 } else {
    			 jsonResult = new JsonResult(resultCode);
    	     }
    	}
        return jsonResult;
    }
